...
(
    ...
    entry_exit_status string comment '进退场状态编码',
    entry_exit_name   string comment '进退场状态名称',
    ...
    fillunit_id       string comment '填报单位id',
    formname          string comment '填报单位名称',
...
select t1.id                                                                          as eeid              --进退场id
     , t1.asid                                                                        as asid              --资产id(非直取,请注意查看文档进行调整)
     ...
     , t1.equ_sour                                                                    as equ_sour_name     --装备来源名称(非直取,请注意查看文档进行调整)
     , t1.equ_code                                                                    as equ_code          --装备编码
     , t1.equ_name                                                                    as equ_name          --装备名称
     , t1.equtype                                                                     as equtype_code      --装备类型编码
     , t1.equtypename                                                                 as equtype_name      --装备类型名称(非直取,请注意查看文档进行调整)
     ...
     , t1.entry_exit_status                                                           as entry_exit_status --进退场状态编码
     , t3.null                                                                        as entry_exit_name   --进退场状态名称(非直取,请注意查看文档进行调整)
     ...
     , t1.formcoid                                                                    as fillunit_id       --填报单位id
     , t1.formname                                                                    as formname          --填报单位名称
     ...
     , ',ods_cccc_erms_base_pub_assetsinfo_i_d,ods_cccc_erms_use_entry_exit_info_i_d' as source_table
from t1
         left join t2
         left join t3
-- on  关联条件以及调整主表顺序
