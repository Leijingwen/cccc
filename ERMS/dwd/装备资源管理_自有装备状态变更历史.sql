...
select c.mastercode                                                                                                      as equ_mastercode     --装备主数据编码(非直取,请注意查看文档进行调整)
     , a.id                                                                                                              as state_change_id    --状态变更id(非直取,请注意查看文档进行调整)
     , a.asid                                                                                                            as asid               --资产id
     , a.status_value                                                                                                    as status_value_code  --状态值编码
     , b.name                                                                                                            as status_value_name  --状态值名称(非直取,请注意查看文档进行调整)
     ...
     , 'stg_cccc_erms_waf_core_dictitem_f_d,ods_cccc_erms_own_state_change_his_i_d,ods_cccc_erms_base_pub_assetsinfo_i_d' as source_table
from a
         left join b on a.status_value = b.code
